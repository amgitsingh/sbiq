...
| 66 | **Rewrite send route** | `POST /send` calls `send_match_email`'s existing logic directly, plus writes a new `EmailLog` row (new audit trail). Test: send a real match email; confirm it sends and an `EmailLog` row is created. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 18 / 24 |
