...
| # | Task | Description | Status |
|---|---|---|---|
| 62 | **Rewrite Studio events routes** | `POST/GET /events`, `GET /events/{id}` call QBCals' existing `create_event`/`list_events`/`_get_event_or_404` logic directly instead of `httpx`-calling itself — now including Task 59's ownership enforcement, not a bare pass-through. Test: create + list + get through the new path; identical shape to the old proxied response, and ownership scoping from Task 59 still holds through this route too. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 14 / 24 |
