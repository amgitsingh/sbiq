...
| # | Task | Description | Status |
|---|---|---|---|
| 69 | **Merge app factory** | Merge IndMatchmaking's `create_app()` (CORS middleware, router registration) into QBCals' `main.py` — every ported router added via `include_router`. Test: `/docs` shows every route (QBCals-native + every ported domain) in one Swagger UI; app boots with one `uvicorn main:app` command. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–72 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access, full route-by-route parity audit | 22 / 25 |
