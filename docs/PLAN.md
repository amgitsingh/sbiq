...
| 49 | **Async DB session** | New `app/core/async_database.py`: async engine + `AsyncSession` factory + `get_async_db` FastAPI dependency, pointed at the same `DATABASE_URL` as the existing sync engine — one physical database, two SQLAlchemy access styles. Test: a throwaway script opens an async session and runs `SELECT 1`. | `done` |
...
| 53 | **Consolidation migration** | One new Alembic migration (continuing QBCals' existing chain) creating every table from #50–52 in QBCals' database. Do not replay IndMatchmaking's own 15 migrations — this single migration recreates their end-state directly. Test: `alembic upgrade head` succeeds on a fresh test DB; `downgrade -1` then `upgrade head` round-trips clean. Applied to the real dev database afterward (purely additive, no data-loss risk) since the ORM models from Task 52 require it to keep working. | `done` |
| 54 | **Checkpoint: real data?** | Confirm with the user whether IndMatchmaking's live database has real rows worth preserving (real registered users, etc.). Only if yes, write a one-off copy script before cutover. Decision checkpoint, not a code change. **Answered (2026-08-26): fresh setup, no data worth preserving — no copy script needed.** | `done` |

### Stage 8.3 — Auth port

| # | Task | Description | Status |
|---|---|---|---|
| 55 | **Port JWT + password hashing** | `app/services/auth/security.py` (PyJWT encode/decode, passlib PBKDF2-SHA256); add `JWT_SECRET_KEY`/`JWT_ALGORITHM`/`JWT_ACCESS_TOKEN_EXPIRE_MINUTES` to `app/core/config.py`. Test: hash+verify a password; encode+decode a token and confirm claims round-trip. | `done` |
| 56 | **Port login/me/current_admin** | `POST /auth/login`, `GET /auth/me` → `app/routers/auth.py`; `current_admin` dependency → `app/services/auth/deps.py` (async, using `get_async_db`). Test: seed one `UserMaster` row, log in, get a token, call `/auth/me`, confirm the right user comes back; wrong password / inactive user rejected. | `done` |
| 57 | **Port registrations + account** | `domain/registrations` → `app/routers/registrations.py`; `domain/account` → `app/routers/account.py`. Test: public signup creates a `pending` user; activate/reject flip status correctly; profile GET/PUT round-trips. Surfaced and fixed along the way: `EmailLog`'s schema (Task 51) only had participant-pair FKs, but registration/activation emails are user-to-user — added `sender_user_id`/`receiver_user_id` back (migration `0010`); pulled `encrypt_credential`/`decrypt_credential` forward from Task 60 (activation email needs it); 3 columns explicitly written with `datetime.now(UTC)` needed to become tz-aware (migration `0011`, asyncpg rejects the naive/aware mismatch outright). Ported Microsoft Graph mail as an optional alternate send path (degrades gracefully, unconfigured by default). | `done` |
| 58 | **Generic role dependency** | New `require_role(*role_names)` dependency factory (IndMatchmaking only had inline `role.role_name` checks). Test: a route guarded by `require_role("Super Admin")` 403s a non-admin token, 200s an admin token. | `done` |
| 59 | **Owner-scoped event access on QBCals' native routers** | Real gap identified live (2026-08-26): QBCals has no auth/ownership today, and IndMatchmaking's old "Super Admin sees all events, others see only their own" access control lived entirely in *its* proxy layer — deleting the proxy (Stage 8.5) without this task would silently remove that access control rather than preserve it. Added `Depends(current_admin)` + new `current_user_role_name` dependency to all 14 event-scoped routes in `app/routers/events.py`; `create_event` stamps `owner_user_id = current_user.id`; `list_events` filters to the caller's own events unless Super Admin; `_get_event_or_404` (the shared choke point ~12 routes go through) 403s when `event.owner_user_id != current_user.id` and the caller isn't Super Admin. Confirmed FastAPI supports a sync route depending on an async dependency natively (no threadpool bridge needed - that's only for Stage 8.5's reverse direction) with a standalone test before the bulk edit. Initial version left unowned (pre-existing) events accessible to everyone as a legacy-data safety net; **amended same day at explicit user request to be strict instead** ("no additional events visible to any user... this is a fresh start") - the unowned-event carve-out was removed from both `_get_event_or_404` and `list_events`, and all pre-Task-59 legacy data (7 events, 364 participants, 244 matches, 1332 enrichment jobs, 167 embeddings, 92 cached enriched profiles) was deliberately wiped so the stricter rule applies cleanly with nothing orphaned. Test (re-run after the amendment): real HTTP test with seeded users - owner reaches their own event and sees it in their list; a directly-inserted unowned event is invisible/403 to a normal user but reachable by Super Admin; Super Admin's list includes it, a normal user's doesn't. | `done` |

### Stage 8.4 — Admin domains port

| # | Task | Description | Status |
|---|---|---|---|
| 60 | **Port simple admin domains** | `domain/dashboard`, `domain/lookups`, `domain/tables`, `domain/smtp`, `domain/profiles` (MatchProfile, carried over as-is) → matching `app/routers/*.py`, same `Depends(current_admin)` router-level gating. `tables.py`'s `TABLE_MODELS` adapted to drop the 5 shadow tables this merge eliminated (`EventMaster`, `ExcelRawData`, `ExcelUpload`, `MatchScoreLog`, `ParticipantMatchMaster`) and their EventMaster-specific ownership branches; QBCals' own `Event`/`Participant`/`Match` deliberately not added to this generic dict-based editor, since they already have purpose-built validated endpoints in `events.py` (Task 59's ownership enforcement included) that a raw editor would bypass. Added `test_microsoft_graph_mail_credentials` to `microsoft_graph_mail.py` (needed by the SMTP test endpoint, missed in Task 57's port). Test: real HTTP test with a seeded Super Admin - lookups returns the static option sets; dashboard counts update 0→1 as a profile is created; profile create/list/get/update/delete all round-trip; SMTP settings saved with real Hostinger credentials and a real live connection test succeeds; tables metadata lists exactly the 10 remaining tables; `smtp_master` records mask `password_encrypted` as null; the dropped `event_master` table correctly 404s. | `done` |
| 61 | **Port external API** | `domain/external` (`EXTERNAL_API_KEY`-gated) → `app/routers/external.py`, repointed to read QBCals' real `Event`/`Participant` tables directly instead of proxied data. Fully sync (no threadpool bridge needed - nothing here touches the async/RBAC side). Deliberately unscoped by owner - a system-to-system API key, not a per-user session, matching its original design. Added `EXTERNAL_API_KEY` (`SecretStr \| None`) to `Settings`. Test: real HTTP calls - open by default with no key configured; once configured, missing/wrong `X-API-Key` 401s and the correct one 200s; real event+2 participants (one single-name, one flagged) correctly mapped across events/participants/csv/detail endpoints, including first/last name splitting and `review`+`enrichment_failed` flag derivation; nonexistent event 404s. | `done` |

### Stage 8.5 — Delete the proxy (the core of the merge)

| # | Task | Description | Status |
|---|---|---|---|
| 62 | **Rewrite Studio events routes** | New `app/routers/studio_events.py`, prefix `/studio` (drops the `/api` prefix, same convention as every other ported router since Task 56), registered in `main.py`. `GET /studio/events`, `POST /studio/events`, `GET /studio/events/{id}` call QBCals' own `list_events`/`create_event`/`_get_event_or_404` (`app/routers/events.py`) as plain Python function calls — no `httpx`, no second HTTP hop — resolving/passing the same `current_admin`/`current_user_role_name` dependencies, so Task 59's owner-scoped access control applies automatically rather than being re-implemented. Response/request shapes adapted to the Studio contract (`StudioEvent`/`StudioEventList`/`StudioEventCreate`, inline in the new file): `target_sectors` null→`[]` via a `field_validator`, `target_sectors`/`event_type`/`content_language` validated as real enums on create (stricter than the native `EventCreate`, matching the original Studio schema's behavior). Only the 3 events routes are in scope — upload/enrich/embed/match/matches/review/send follow in Tasks 63-65. Found and fixed along the way: this sandbox's Python 3.14 interpreter has no prebuilt wheels yet for `asyncpg`/`lxml`/`greenlet`/`pydantic-core` (source build fails, no MSVC linker) — switched to the Python 3.12 interpreter already on the box and reinstalled `requirements.txt` there; this is an environment note, not a code change. Test: real HTTP test via `httpx.AsyncClient(ASGITransport)` against the live local Postgres DB — seeded two `Admin`-role users and one `Super Admin`; `POST /studio/events` (user1) returns 201 with the exact `StudioEvent` shape (`target_sectors: ["technology","finance_banking"]`, `event_type: "networking"`); `GET /studio/events` (user1) lists it, `GET /studio/events/{id}` (user1) returns it, and the native `GET /events` shows the same row (parity with the old proxied response); user2 (different owner, same role) gets neither the event in their list nor direct access (`403`); Super Admin sees it in the list and via direct `GET`; an unknown id 404s. All test rows cleaned up afterward (DB back to empty). | `done` |
| 63 | **Rewrite Studio upload route** | `POST /studio/events/{id}/upload` (added to `app/routers/studio_events.py`) calls QBCals' own `upload_participants` (`app/routers/events.py`, which itself calls `run_ingestion_pipeline`) directly as a plain function call - no intermediate pre-checks (empty-file/size-cap/suffix-allowlist) beyond what the native route already enforces, since re-adding IndMatchmaking's old ones here could reject/accept a file differently than the native path would for the same input. New `StudioUploadSummary` mirrors the native `UploadSummary` field-for-field (pure pass-through, not a re-derived shape). Test: real HTTP test - uploaded a real 2-row CSV through the new `/studio/events/{id}/upload` path, confirmed the response summary (`total_rows`/`valid`/`rejected`/etc.) and that the participants actually landed (checked via native `GET /events/{id}/participants`); uploaded an equivalent CSV through the native `/events/{id}/upload` path and confirmed byte-for-byte identical summary shape for equivalent input; confirmed an empty file 400s through the new path exactly as it does natively. All test data cleaned up afterward. | `done` |
| 64 | **Rewrite enrich/embed/match routes** | `POST/GET /studio/events/{id}/enrich`, `/enrichment-status`, `/embed`, `/embedding-status`, `/match`, `/matching-status` (added to `studio_events.py`) call QBCals' own `trigger_enrichment`/`get_enrichment_status`/`trigger_embedding`/`get_embedding_status`/`trigger_matching`/`get_matching_status` directly. Kept as plain `def` handlers (not `async def`) - the plan originally called for `run_in_threadpool`/`asyncio.to_thread` bridging, written when these routes were expected to stay `async def` like the old httpx-proxying versions; since they now call QBCals' own sync functions directly instead of `await`-ing an HTTP client, there's nothing async to bridge (FastAPI already runs a sync `def` route in its threadpool, same as events.py's own trigger_* functions) - a deliberate, verified simplification over the original plan wording. New Studio*/embed/match schemas mirror the native ones field-for-field. Found along the way: this sandbox had no Redis/Celery worker running (needed for real `.delay()` dispatch in enrichment/matching) - a `redis:7` Docker container was already present but stopped, so it was started, and a real `celery -A app.workers.celery_app worker -Q enrichment,matching` was run temporarily for this test only (stopped afterward - not a permanent change). Test: real HTTP test, no mocking - ran a full real enrich→embed→match(cost estimate) cycle for one test participant on two twin events, one driven entirely through the native `/events/*` routes and one entirely through the new `/studio/events/*` routes; both used real Tavily/OpenAI calls (the studio-path run hit `ENABLE_ENRICHMENT_REUSE`'s cross-event cache and reused the native run's just-computed profile by email, exercising that feature for real too); enrichment/embedding status and cost-estimate breakdowns matched field-for-field between the two paths; `confirm=true` on both paths correctly enqueued a real Celery job (202 + real `job_id`, picked up and executed by the live worker) - identical outcome on both paths. All test data (events, participants, the enrichment-reuse cache row) cleaned up afterward; DB verified empty. | `done` |
| 65 | **Rewrite review route** | `POST /studio/events/{id}/review` (added to `studio_events.py`) writes `Match.status`/`reviewed_by_user_id`/`reviewed_at` directly - unlike every other Task 62-64 route, there's no native events.py function to delegate to (QBCals never had its own review endpoint; "review" was purely an IndMatchmaking concept, now folded onto `Match` itself per the merge design), so this is genuinely new logic, not a call-through. Looks the `Match` row up by `(event_id, participant_a_id=recipient_id, participant_b_id=counterpart_id)`, maps `decision` ("approve"/"reject") to `status` ("approved"/"rejected"), stamps `reviewed_by_user_id`/`reviewed_at`. 404s on an unknown pair. **Real bug found and fixed along the way**: `app/models/__init__.py` never imported the Phase 8 merge's new models (`UserMaster`, `RoleMaster`, etc.) - the FastAPI app process never noticed (its router-import chain touches every model anyway), but the Celery worker process (imports only `app.workers.*`) hit `NoReferencedTableError` resolving `matches.reviewed_by_user_id`'s FK to `user_master` at commit time inside `match_participant`, since SQLAlchemy only resolves a string-based FK target against tables whose model has actually been imported into that process. Fixed by adding `UserMaster`/`RoleMaster`/`PermissionMaster`/`RolePermissionMapping`/`CompanyMaster`/`TagMaster`/`UserTagMapping`/`EventParticipantMapping`/`SmtpMaster`/`MatchProfile`/`EmailLog` to `app/models/__init__.py` - this was a real latent bug in the merge (present since Task 51-59, just never previously exercised by a worker-process commit touching those columns), not scope creep. Also fixed a stale "Task 64" comment reference in `app/models/match.py` (renumbered to 65 when the plan was restructured). Test: real HTTP test - started a real Celery worker (`-Q enrichment,matching`) against the already-running local `redis:7` Docker container (stopped again afterward); ran a full real enrich→embed→match(confirm=true) cycle for two complementary participants (an investor + a fintech founder) through the new `/studio` routes, producing one genuine LLM-generated match with real reasoning/email/LinkedIn drafts; approved it via the new review route and confirmed `matches.status`/`reviewed_by_user_id`/`reviewed_at` updated on exactly that row (verified via direct SQL); reviewed it again as `reject` and confirmed `status` flipped to `rejected`; confirmed an unknown participant pair 404s. All test data cleaned up afterward; DB verified empty. | `done` |
| 66 | **Rewrite send route** | `POST /studio/events/{id}/send` (added to `studio_events.py`) calls QBCals' own `send_match_email` (`app/routers/events.py`) directly for the real SMTP send - goes to `participant_b`/`counterpart_id`, framed as coming from `participant_a`/`recipient_id` ("introduction from A to B" - the native route's own design, not IndMatchmaking's old per-user-SMTP/Microsoft-Graph flow, which sent the draft back to the recipient themselves instead; superseded per this task's "call the existing logic directly" instruction) - then writes a new `EmailLog` row as an audit trail, which the native route itself never did. `recipient_id`/`counterpart_id` field names kept consistent with Task 65's review route (`recipient_id` = `participant_a`, the match's own selector), even though the actual email lands in `counterpart_id`'s inbox - documented in the route's docstring to avoid confusion. Logs `event_id`/`match_id`/`sender_participant_id`/`receiver_participant_id`/`subject`/`body`(the real `email_draft`)/`status`/`sent_at`; on a native `HTTPException` (e.g. missing email/draft) with a Match row still resolvable, logs `status="failed"` with the error instead of silently dropping the audit trail. Test: real HTTP test, no mocking - full real enrich→embed→match→review cycle (same investor/founder pair pattern as Task 65) producing a genuine match with a real `email_draft`; sent it via the new route, confirming a real SMTP send succeeded (Hostinger) and the `EmailLog` row was created with every field correct (event/match/participant ids, subject, body matching the actual draft, `status="sent"`, `sent_at` populated); sent the same pair again and confirmed a second `EmailLog` row was added (native `send_match_email` has no dedup guard of its own, so this wrapper correctly doesn't invent one); an unknown participant pair 404s with no `EmailLog` row written (no Match row to attach one to). All test data cleaned up afterward; DB verified empty. | `done` |
| 67 | **~~Delete the proxy client~~ → Self-sufficiency pass** | **Reframed at explicit user direction**: *"no need to delete anything from indmatchmaking app, just make sure SBIQ is self sufficient and has all the endpoints that is needed to work with users and events and matchmaking."* The original text assumed IndMatchmaking would survive as the primary app with its own `lib/qbcals.py`/`domain/studio/*` deleted once it called QBCals in-process - but this merge instead made **QBCals/SBIQ itself** the surviving merged backend (Tasks 48-66 ported every IndMatchmaking domain natively into `app/routers/*.py`), so `lib/qbcals.py` and `QBCALS_API_BASE_URL` only ever existed in the separate IndMatchmaking repo, which still actively registers its own `/api/studio/*` router - deleting them there now would break that still-live app for no benefit, since nothing in SBIQ depends on them and nothing has cut IndMatchmaking's traffic over yet (that's Stage 8.6/Task 68's concern). Left entirely untouched, by instruction. Instead, audited SBIQ's own native surface for real gaps and fixed two: (1) `Event.location` (a real column, added back in Task 59 for IndMatchmaking parity) was never wired into `EventCreate`/`EventOut` in `events.py` nor into `StudioEvent`/`StudioEventCreate` in `studio_events.py` - added to all four; (2) QBCals had no native single-event `GET`/`PATCH`/`DELETE /events/{event_id}` at all (only reachable indirectly via the Studio wrapper's `GET`) - added all three directly to `events.py`, with Task 59's ownership enforcement via `_get_event_or_404`, placed after `GET /participants-template`'s registration so a bare `/{event_id}` pattern doesn't shadow that literal path. `DELETE` uses the same raw-SQL-delete pattern already established everywhere in this merge for the known `participant_embeddings` ORM-cascade bug, now for the first time in real application code rather than test cleanup. No Studio-side `PATCH`/`DELETE` were added - the original IndMatchmaking Studio contract never had them, so there's no compatibility shape to preserve. Test: real HTTP test - `location` round-trips through create/read on both `/events` and `/studio/events`; native `GET/PATCH/DELETE /events/{id}` all correctly 403 for a non-owner, 404 for an unknown id; `PATCH` partial updates leave untouched fields alone across two separate partial patches; `DELETE` really removes the row (`GET`/`PATCH`/`DELETE` on it all 404 afterward, confirmed via direct SQL count too); confirmed `GET /events/participants-template` still resolves correctly (route-ordering regression check) rather than being shadowed by the new `/{event_id}` pattern. All test data cleaned up afterward. | `done` |
| 68 | **Path collapse** | **Decision checkpoint, resolved**: asked the user given Task 67's reframing (native `/events/*` and the `/studio/events/*` shim had become two overlapping surfaces for the same functionality, with `/studio` alone holding review/send-with-audit-log) - user chose to collapse now. Folded `POST /{event_id}/review` (`ReviewMatchRequest`/`ReviewMatchResult`, verbatim logic from the deleted Task 65 route) and an `EmailLog` audit-trail write (both `status="sent"` and `status="failed"` paths) directly into `app/routers/events.py`'s existing `send_match_email`. Deleted `app/routers/studio_events.py` entirely (its `/studio/events/*` routes, `StudioEvent*` schemas) and its registration in `main.py`. Route count: 51 (was 62 after Task 67: -12 deleted Studio routes, +1 new native review route). No Studio-side capability was lost - review and the EmailLog audit trail now live natively; nothing depended on the `/studio` prefix itself (IndMatchmaking's own frontend still calls IndMatchmaking directly, untouched per Task 67). Test: real HTTP test, no mocking - confirmed `GET /studio/events` now 404s (the surface is genuinely gone); ran the full real users→events→matchmaking cycle (create-with-location → read → upload → enrich → embed → match(confirm=true, real Celery dispatch) → review → send) entirely through native `/events/*`; verified `Match.status`/`reviewed_by_user_id`/`reviewed_at` update correctly via review; verified the `EmailLog` row from the send path has every field correct (event/match/participant ids, subject, the real draft body, `status="sent"`, `sent_at`); verified `PATCH /events/{id}` still works post-collapse. All test data cleaned up afterward; DB verified empty. | `done` |

### Stage 8.6 — App wiring & deployment

> **Note added 2026-08-27**: the user has since clarified *"this project is a
> replacement for indmatchmaking. Indmatchmaking will not be used anymore.
> SBIQ should have all the apis present, that was there in Indmatchmaking."*
> Tasks 69-71 below were written when the plan still assumed a two-repo
> consolidation (merging IndMatchmaking's `create_app()`/config into QBCals,
> deploying both from one process). Since QBCals/SBIQ is now the sole
> surviving app and IndMatchmaking's repo is not being touched or deployed
> alongside it (see Task 67/68's notes and [[merge_strategy_indmatchmaking]]
> memory), these three re-read as: confirm SBIQ's own `main.py`/config/deploy
> doc are already complete and correct standalone - not "merge two apps'
> wiring together". Re-scope with the user before executing, same as Tasks
> 67-68.

| # | Task | Description | Status |
|---|---|---|---|
| 69 | **Merge app factory** | **Reframed** (see Stage 8.6's note above): no cross-repo merge needed - `main.py` already registers every router as a plain module-level `app` (no `create_app()` factory needed either; nothing in this codebase constructs the app differently for tests). Confirmed router registration is complete (53 routes / 38 distinct paths across all 9 routers, single Swagger UI). Adopted the one genuinely useful piece "merging the app factory" would have brought over: IndMatchmaking's configurable CORS (`ALLOWED_CORS_ORIGINS`/`CORS_ALLOW_METHODS`/`CORS_ALLOW_HEADERS` via `Settings`, not hardcoded `"*"`). **Real bug found and fixed while adding this**: declaring these as `list[str]` Settings fields breaks outright for a comma-separated `.env` value - pydantic-settings tries to JSON-decode any "complex" (list) field's raw env string *before* any `field_validator` runs, so `ALLOWED_CORS_ORIGINS=https://a.com,https://b.com` crashes at `Settings()` construction with a `JSONDecodeError`, not just fails validation. Fixed by keeping these as plain `str` Settings fields and adding `_split_csv()` in `app/core/config.py`, applied at the point of use in `main.py`. Defaults (`"*"`) preserve the app's pre-existing wide-open local-dev behavior exactly - zero-config deployments are unaffected. Test: real HTTP test via `ASGITransport` - default (no env override) CORS still reflects `*`/echoes any origin, exactly as before; with `ALLOWED_CORS_ORIGINS` set to one real origin (a separate process, since `Settings` is a module-level singleton read at import time), an allowed `Origin` gets `Access-Control-Allow-Origin` echoed back and a disallowed one gets no such header at all; confirmed comma-separated multi-value parsing works for origins and methods; `/openapi.json` and `/docs` both reachable and list every domain's routes in one place. | `done` |
| 70 | **Reconcile config** | **Reframed** (same lens as #67-69): no cross-repo config merge needed - `app/core/config.py`'s `Settings` class already had every field this task named (`JWT_*`, `ALLOWED_CORS_ORIGINS`+siblings from #69, `SMTP_ENCRYPTION_KEY`, `MICROSOFT_GRAPH_*`), added incrementally across Tasks 56-69. The actual gap: `.env.example` was never updated to document any of them - it still ended at the original Phase-1-era SMTP section, so a fresh clone had no way to discover `JWT_SECRET_KEY`/`EXTERNAL_API_KEY`/etc. existed without reading `config.py`'s source. Added 6 new documented sections (Auth/JWT, per-user SMTP credential encryption, Microsoft Graph mail, registration links, external API, CORS) covering every field added since the merge began. Checked `CONTACT_*` specifically: confirmed via grep that IndMatchmaking itself declares `CONTACT_NAME`/`CONTACT_EMAIL` in its own `Settings` but never actually reads them anywhere in its own codebase - dead config even there, so not ported (no capability behind it to replicate). "Drop proxy-specific vars removed in #67" doesn't apply - `QBCALS_API_BASE_URL`/etc. never existed in SBIQ's `Settings` to begin with (see #67's note). Test: real check, no mocking - loaded a byte-for-byte fresh copy of the updated `.env.example` into a `Settings()` instance and confirmed every field (including the 6 new sections) resolves with the correct documented defaults and no construction errors; booted the actual `main.py` app object against that fresh config (plus a real `DATABASE_URL`, since nothing else needs a live DB connection just to import) and confirmed all 53 routes still register with zero missing-config errors. | `done` |
| 71 | **Update deployment doc** | **Reframed** (same lens as #67-70): `docs/DEPLOY_EC2.md` already documented exactly one systemd service group (API + 2 Celery workers) for SBIQ alone - it never described a second app to consolidate, since IndMatchmaking's own deployment (if any) is untouched, separate infrastructure. **Real deployment-blocking bug found while checking it against everything Phase 8 added**: a genuinely fresh database (`alembic upgrade head` on empty tables) has zero rows in `role_master`/`user_master`, and *every* way to create or approve a user requires either the `"User"` role to already exist (`POST /public/register` 500s without it) or an already-logged-in Super Admin (`POST /admin/registrations/{id}/activate`) - there was no seed/bootstrap mechanism anywhere in the codebase, so a fresh deploy had no way into the app via the API at all. IndMatchmaking has the identical problem and solves it with `app/db/seed.py` (hardcoded default credentials, fine for local dev). Ported the concept as new `backend/seed_admin.py` - idempotent, real CLI-provided email/password (no hardcoded default, since this targets real deployments), seeds the 3 core roles plus one active Super Admin. Documented as a new required Step 6.5 in `docs/DEPLOY_EC2.md`, plus a `JWT_SECRET_KEY`-must-change warning (Step 5) and an `ALLOWED_CORS_ORIGINS` update (Task 69) in the "things worth knowing" section. **Scope note**: literal EC2 redeployment wasn't performed in this sandboxed session (no AWS credentials/spend authorization) - instead, did the most thorough substitute possible: full route/config audit against a real local Postgres DB, finding and fixing the one genuine blocker such a redeploy would have hit. Test: real check, no mocking - confirmed the local DB had 0 users beforehand (2 of 3 core roles already existed from earlier tasks' testing, `"User"` did not - a real, live instance of the exact bug this fixes); ran `seed_admin.py` for real, confirmed all 3 roles + one Super Admin created; logged in via the real `/auth/login` endpoint with the seeded credentials, confirmed `/auth/me` returns the right user with `role_name: "Super Admin"`; used that session to register a new user via the real `/public/register` and successfully call `/admin/registrations/{id}/activate` on it (hit the expected, correct 400 for SMTP not being configured yet - proving the chain works right up to that real precondition); ran the script a second time and confirmed idempotency (existing roles/user reported, not duplicated or modified). All test data cleaned up afterward. | `done` |

### Stage 8.7 — IndMatchmaking full-parity audit

| # | Task | Description | Status |
|---|---|---|---|
| 72 | **Route-by-route parity audit + fixes** | Triggered by the user's explicit clarification that IndMatchmaking is being fully retired and SBIQ must have every API it had. Extracted every `@router.*` decorator from all 10 IndMatchmaking domains (`account`, `auth`, `dashboard`, `events`, `external`, `lookups`, `profiles`, `registrations`, `smtp`, `studio`, `tables`) and diffed against SBIQ's live route table. Every domain was already fully covered except IndMatchmaking's own `EventMaster`-based `events` domain (superseded by QBCals' native `Event`/`Participant`, but three real capabilities had never carried over) - confirmed two of three with the user via `AskUserQuestion` before building (both approved, recommended options): **(1)** `GET /events` gained `q` (search across name/location/description/status), `limit`/`offset` pagination (default 50, matching IndMatchmaking's own default), and a Super-Admin-only `user_id` filter - response stays a bare array (not IndMatchmaking's `{items, total}` envelope) to avoid a breaking shape change for the real admin frontend already calling this endpoint; total count instead surfaced via an `X-Total-Count` header. **(2)** Super Admin can now create an event on behalf of another user (`EventCreate.owner_user_id`, 403 for anyone else naming a different owner) and transfer an existing event's ownership (`EventUpdate.owner_user_id`, handled specially in `update_event` since it needs a role check + `_validate_owner` existence check the other plain fields don't) - both validate the target user exists and is active. **(3)** New `ParticipantUploadBatch` model/table (`app/models/upload_batch.py`, migration `0012`) persists every `POST /{event_id}/upload` call's summary (counts, `unmapped_headers`, `rejected_details`, a derived `completed`/`completed_with_errors`/`failed` status) so it's queryable after the fact via new `GET /{event_id}/uploads` (list) and `GET /{event_id}/uploads/{upload_id}` (detail) - restores IndMatchmaking's `ExcelUpload`/`ExcelRawData` capability (one row per upload call, not one row per source Excel row - deliberately not resurrecting those shadow tables verbatim) without which a rejected-row report was previously only visible in the synchronous upload response. `UploadSummary` gained `upload_id` linking the two. Test: real HTTP test, no mocking - non-Super-Admin creating/transferring an event to another user 403s; Super Admin creating for / transferring to another user succeeds and the new owner immediately has access while the old owner loses it; an invalid `owner_user_id` 400s; `q` search correctly filters and reports `X-Total-Count`; `limit`/`offset` truncate correctly; Super Admin's `user_id` filter scopes correctly and is silently ignored for non-Super-Admin; two real uploads (one clean, one with a rejected row) against the same event produce two persisted batches with correct derived statuses, `GET /{event_id}/uploads` lists both, `GET .../uploads/{id}` returns full detail including `rejected_details`, an unknown upload id 404s, and a wholly-empty file (which 400s before any batch is created) does not create a stray batch row. All test data cleaned up afterward; DB verified empty. | `done` |
...
| 8 — Merge with IndMatchmaking | 48–72 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access, full route-by-route parity audit | 25 / 25 |

**Total: 65 tasks** — frontend and admin panel handled separately.
