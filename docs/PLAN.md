...
| # | Task | Description | Status |
|---|---|---|---|
| 60 | **Port simple admin domains** | `domain/dashboard`, `domain/lookups`, `domain/tables`, `domain/smtp`, `domain/profiles` (MatchProfile, carried over as-is) → matching `app/routers/*.py`, same `Depends(current_admin)` router-level gating. Test: each ported router exercised via `/docs` with a real admin token — same responses as the old IndMatchmaking service. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 12 / 24 |
