...
| 61 | **Port external API** | `domain/external` (`EXTERNAL_API_KEY`-gated) → `app/routers/external.py`, repointed to read QBCals' real `Event`/`Participant` tables directly instead of proxied data. Test: real HTTP call with the API key header returns real QBCals data. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 13 / 24 |
