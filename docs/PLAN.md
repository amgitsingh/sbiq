...
| 63 | **Rewrite Studio upload route** | `POST /events/{id}/upload` calls `run_ingestion_pipeline` directly. Test: upload a real test file; participants land exactly as QBCals' own upload endpoint would produce. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 15 / 24 |
