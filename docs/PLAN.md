...
| 71 | **Update deployment doc** | Update `docs/DEPLOY_EC2.md` for one systemd service instead of two separately-deployed apps. Test: redeploy against a real test instance following the updated doc; `/docs` and a ported auth endpoint both reachable from one process. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–72 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access, full route-by-route parity audit | 24 / 25 |
