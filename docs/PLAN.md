...
| 65 | **Rewrite review route** | `POST /review` writes `Match.status` + `reviewed_by_user_id`/`reviewed_at` instead of the old `MatchReview` table. Test: review a real match; confirm the three columns update on the right `Match` row. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 17 / 24 |
