...
| 59 | **Owner-scoped event access on QBCals' native routers** | Real gap identified live (2026-08-26): QBCals has no auth/ownership today, and IndMatchmaking's old "Super Admin sees all events, others see only their own" access control lived entirely in *its* proxy layer — deleting the proxy (Stage 8.5) without this task would silently remove that access control rather than preserve it. Add `Depends(current_admin)` to `app/routers/events.py`; `create_event` stamps `owner_user_id = current_user.id`; `list_events` filters `WHERE owner_user_id == current_user.id` unless the caller is Super Admin; `_get_event_or_404` (the shared choke point ~20 event-scoped routes already go through — upload, enrich, embed, match, matches, participant detail, etc.) 403s when `event.owner_user_id != current_user.id` and the caller isn't Super Admin. FastAPI supports a sync route depending on an async dependency natively (no threadpool bridge needed here — that's only for Stage 8.5's reverse direction). Test: as a non-Super-Admin user, create an event, confirm it's the only one `list_events` returns; confirm a second such user gets 403/404 on the first user's event id via every route that calls `_get_event_or_404`; confirm Super Admin still sees/reaches everything. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 11 / 24 |
