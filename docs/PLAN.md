...
| 70 | **Reconcile config** | Merge `JWT_*`, `ALLOWED_CORS_ORIGINS`, `SMTP_ENCRYPTION_KEY`, `MICROSOFT_GRAPH_*`, `CONTACT_*` into `.env.example`/`Settings`; drop proxy-specific vars removed in #67. Test: fresh `.env` from the updated `.env.example` boots the app with no missing-config errors. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–72 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access, full route-by-route parity audit | 23 / 25 |
