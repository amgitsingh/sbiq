...
| 68 | **Checkpoint: path collapse?** | Decide with the user whether to collapse `/api/studio/*` into QBCals' native `/events/*` path space (recommended — the "studio vs native" distinction no longer means anything post-merge). Breaking change for any frontend code still calling `/api/studio/...`. Decision checkpoint. | `pending` |
...
| 8 — Merge with IndMatchmaking | 48–71 | Fold IndMatchmaking's auth/user-management/admin domains into QBCals, delete the HTTP proxy hop, consolidate into one database, enforce owner-scoped event/match access | 20 / 24 |
